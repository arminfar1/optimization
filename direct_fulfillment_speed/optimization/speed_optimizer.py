...
class Optimize:
    ...
    def __init__(
        ...
        self.min_dea_furniture_ups_ground = self.config.min_dea_furniture_ups_ground
        self.min_dea_furniture_swa = self.config.min_dea_furniture_swa
        self.min_dea_tires_ups_ground = self.config.min_dea_tires_ups_ground
    ...
    def _create_decision_variables(self, is_third_party):
        ...
        for entity, possible_pads in self.ods_prediction.items():
            ...
            # Initialize max_pad with a default value
            max_pad = self.config.max_pad_swa  # Default for SWA

            if is_third_party and isinstance(entity, ODS):
                # For 3P, check shipment type and corresponding threshold
                if entity.carrier.shipment_type == ShipmentType.UPS_GROUND:
                    if (
                        (self.min_3p_ground_dea <= 0)
                        and (self.min_dea_furniture_ups_ground <= 0)
                        and (self.min_dea_tires_ups_ground <= 0)
                    ):
                        continue
                    max_pad = self.config.max_pad
                elif entity.carrier.shipment_type == ShipmentType.UPS_AIR:
                    if self.min_3p_air_dea <= 0:
                        continue
                    max_pad = self.config.max_pad_air
                else:
                    continue  # Skip if neither ground nor air

            self._set_up_decision_variable(
                entity=entity,
                possible_pads=possible_pads,
                min_pad=min_pad,
                max_pad=max_pad,
    ...
    def _set_up_decision_variable(
        self,
        entity: Union[ODS, Warehouse],
        possible_pads: Dict[float, float],
        min_pad: float,
        max_pad: float,
        is_third_party: bool,
    ) -> None:
